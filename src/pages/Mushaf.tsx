...
const Mushaf: React.FC = () => {
  ...
  const [mode, setMode] = useState<"image" | "text">(
    () => (localStorage.getItem("mushaf-mode") as "image" | "text") || "image"
  );
  const [page, setPage] = useState(() => {
    const p = pageParam ? parseInt(pageParam) : parseInt(localStorage.getItem("mushaf-page") || "1");
    return Math.max(1, Math.min(p, TOTAL_PAGES));
  });
  const [loading, setLoading] = useState(false);
  const [textData, setTextData] = useState<any>(null);
  const [bookmarks, setBookmarks] = useState<number[]>(() => {
    try { return JSON.parse(localStorage.getItem("mushaf-bookmarks") || "[]"); } catch { return []; }
  });
  const [imgError, setImgError] = useState(false);

  useEffect(() => {
    localStorage.setItem("mushaf-page", String(page));
  ...
  useEffect(() => {
    if (mode === "text") {
      setLoading(true);
      QuranAPI.getPage(page)
        .then((d) => setTextData(d))
        .catch(() => setTextData(null))
  ...
  const toggleBookmark = () => {
    ...
    localStorage.setItem("mushaf-bookmarks", JSON.stringify(updated));
  };
    ...
    <div className="px-4 py-4" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="flex items-center gap-2 mb-4 animate-fade-in">
        <div className="flex flex-1 bg-card rounded-xl p-1 border border-gold/10">
          <button
            ...
            className={`flex-1 py-2 text-xs font-medium rounded-lg transition-smooth ${mode === "image" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
            📷 Image
          </button>
          <button
            onClick={() => setMode("text")}
            className={`flex-1 py-2 text-xs font-medium rounded-lg transition-smooth ${mode === "text" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
          >
            📝 Text
          </button>
        </div>
      ...
      {mode === "image" ? (
        <div className="rounded-2xl overflow-hidden border border-gold/10 shadow-gold bg-card min-h-[400px] flex items-center justify-center">
          {imgError ? (
            <div className="text-center py-12">
              <p className="text-muted-foreground text-sm mb-2">Failed to load page</p>
              <button onClick={() => setImgError(false)} className="text-gold text-sm underline">Retry</button>
            </div>
          ) : (
            <img
              src={QuranAPI.getMushafPageImage(page)}
              alt={`Quran Page ${page}`}
              className="w-full"
        ...
        <div className="rounded-2xl p-4 bg-card border border-gold/10 shadow-gold" dir="rtl">
          <div className="font-arabic text-arabic text-lg leading-[2.4] text-center">
            {textData.ayahs?.map((a: any) => (
              <span key={a.number}>
                {a.text}{" "}
                <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-primary/20 text-gold text-[10px] align-middle mx-1">
                  {a.numberInSurah}
                </span>{" "}
              </span>
            ))}
