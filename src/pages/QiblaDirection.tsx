...
const QiblaDirection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const getLocation = async () => {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          if (!cancelled) {
            setLocationError("Location permission denied. Please enable GPS to find Qibla direction.");
          }
        },
  ...
  const handleOrientation = useCallback((e: DeviceOrientationEvent) => {
    ...
    if (!rafRef.current) {
      rafRef.current = requestAnimationFrame(() => {
        smoothHeadingRef.current = smoothAngle(smoothHeadingRef.current, headingRef.current);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!hasCompass) {
        setCompassError("Compass sensor not detected on this device. Rotate your phone in a figure-8 pattern to calibrate.");
        setCalibrating(false);
      }
    }, 4000);
        ...
        <>
          ...
          {calibrating && hasCompass && (
            <div className="mb-3 flex items-center gap-2 px-4 py-2 rounded-xl bg-accent/50 animate-fade-in">
              ...
              <p className="text-xs text-muted-foreground">Calibrating compass... Move your phone in a figure-8</p>
            </div>
          )}
          ...
          <div className="relative w-72 h-72 mb-6 animate-fade-in">
            ...
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <div className={`w-3 h-3 rounded-full shadow-lg transition-colors duration-300 ${isPointingQibla ? "bg-green-500" : "bg-primary"}`} />
            </div>
          </div>
