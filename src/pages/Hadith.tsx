  ...
  const openBook = useCallback(async (book: HadithBook) => {
    ...
    try {
      ...
      if (offline) {
        ...
        const edition = (lang === "urdu" || lang === "hindi") ? book.urduEdition : book.englishEdition;
            ...
            {view.type !== "books" && !showDownloadManager && (
              <div className="flex gap-1">
                {(["english", "urdu", "hindi"] as const).map((l) => (
                  <button key={l} onClick={() => { setLang(l); localStorage.setItem("hadith_book_lang", l); }} className={`text-[9px] px-2 py-0.5 rounded-full font-medium transition-smooth ${lang === l ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{l === "english" ? "EN" : l === "urdu" ? "UR" : "HI"}</button>
                ))}
              </div>
            )}
          </div>
          ...
          {filteredHadiths.map((h) => {
            ...
            return (
              <div key={h.hadithnumber} className="rounded-2xl bg-card border border-border overflow-hidden">
                ...
                <div className="px-4 pb-3">{lang === "urdu" && urd ? <p className="text-sm text-muted-foreground leading-relaxed text-right font-urdu" dir="rtl">{urd.text}</p> : lang === "hindi" && urd ? <p className="text-sm text-muted-foreground leading-relaxed">{urd.text}</p> : <p className="text-sm text-muted-foreground leading-relaxed">{h.text}</p>}</div>
